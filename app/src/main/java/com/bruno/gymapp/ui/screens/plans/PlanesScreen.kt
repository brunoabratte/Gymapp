...
@Composable
fun PlanesScreen(onPlanClick: (String) -> Unit) {
    Scaffold(topBar = { TopAppBar(title = { Text("Elegí tu plan") }) }) { padding ->
        LazyColumn(
...
private fun PlanCard(plan: PlanPreconfigurado, onClick: () -> Unit) {
    Card(modifier = Modifier.fillMaxWidth().clickable(onClick = onClick)) {
        Column(Modifier.padding(16.dp)) {
            ...
            Text(
                "Compuestos ${plan.descansoCompuesto}s  •  Básicos ${plan.descansoBasico}s  •  Aislamiento ${plan.descansoAislamiento}s",
